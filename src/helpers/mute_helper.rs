...
pub async fn prepare_mute_timer(ctx: &Context, user_id: UserId, guild_id: GuildId, mute_time: u64) -> CommandResult {
    let pool = ctx.data.read().await.get::<ConnectionPool>().cloned().unwrap();
    let current_time = SystemTime::now().duration_since(UNIX_EPOCH).expect("Time can't go backwards.");
    let unmute_time = current_time.as_secs() + mute_time;
    sqlx::query!(
        "INSERT INTO mutes VALUES($1, $2, $3) ON CONFLICT (guild_id, user_id) DO UPDATE SET mute_time = EXCLUDED.mute_time",
        guild_id.0 as i64, user_id.0 as i64, unmute_time as i64
    ).execute(&pool).await?;
    let ctx_clone = ctx.clone();
    tokio::spawn(async move {
        create_mute_timer(ctx_clone, mute_time, user_id, guild_id).await;
    });
    Ok(())
}
...
pub async fn unmute_by_time(ctx: &Context, user_id: &UserId, guild_id: &GuildId) -> CommandResult {
    ...
    let guild = match ctx.cache.guild(guild_id).await {
        Some(guild) => guild,
        None => {
            error!("There was an error getting guild {} from the cache", guild_id.0);
            return Ok(())
        }
    };
    sqlx::query!(
        "DELETE FROM mutes WHERE guild_id = $1 AND user_id = $2",
        guild.id.0 as i64, user_id.0 as i64
    ).execute(&pool).await?;
    ...
    match member.remove_role(ctx, mute_info.mute_role_id).await {
        ...
        Err(_) => {
            mute_info.mute_channel_id.say(ctx, format!(
                "Failed to remove the mute role from user {} with ID {}", user_id.mention(), user_id
            )).await?;
        }
    };
    Ok(())
...
pub async fn handle_mute_role(ctx: &Context, guild: &Guild, channel_id: Option<ChannelId>, forced_gen: bool) -> CommandResult<MuteInfo> {
    let pool = ctx.data.read().await.get::<ConnectionPool>().cloned().unwrap();
    ...
    if guild.roles.contains_key(&mute_role_id) {
        if forced_gen {
            channel_id.say(ctx, "Role exists, aborting.").await?;
        }
        ...
        channel_id.say(ctx, "The `Muted` role was deleted, recreating...").await?;
        let mute_info = new_mute_role(ctx, guild, channel_id).await?;
        Ok(mute_info)
    }
}

pub async fn new_mute_role(ctx: &Context, guild: &Guild, channel_id: ChannelId) -> CommandResult<MuteInfo> {
    let pool = ctx.data.read().await.get::<ConnectionPool>().cloned().unwrap();
    ...
    let overwrite_voice = PermissionOverwrite {
        ...
        kind: PermissionsOverwriteType::Role(mute_role.id),
    };
...
pub async fn fetch_guild_mutes(pool: &PgPool, guild: &Guild, mute_role_id: RoleId) -> CommandResult<(String, String)> {
    let timed_mutes = fetch_timed_mutes(pool, &guild.id).await?;
    let permanent_mute_test = guild.members.iter()
        .filter(|(u, m)| m.roles.contains(&mute_role_id) && !timed_mutes.contains_key(u))
        .format_with(" \n", |(u, _), f| f(&u.mention())).to_string();
...
pub async fn fetch_timed_mutes(pool: &PgPool, guild_id: &GuildId) -> CommandResult<HashMap<UserId, String>> {
    let mute_data_vec = sqlx::query!("SELECT * FROM mutes WHERE guild_id = $1", guild_id.0 as i64)
        .fetch_all(pool).await?;
    let timed_mutes = mute_data_vec.iter().map(|mute_data| {
        let naive_datetime = NativeDateTime::from_timestamp(mute_data.mute_time, 0);
...
pub async fn load_mute_timers(ctx: &Context) -> CommandResult {
    ...
    for i in timer_data {
        ...
        info!("user {}, guild {}, time {}", &i.user_id, &i.guild_id, mute_time_diff);
        let guild_id = GuildId::from(i.guild_id as u64);
        let user_id = UserId::from(i.user_id as u64);
        if mute_time_diff <= 0 {
            let check = sqlx::query!(
                "SELECT EXISTS(SELECT 1 FROM delete_time_store WHERE guild_id = $1)", i.guild_id
            ).fetch_one(&pool).await?;
            if !check.exists.unwrap() {
                info!("Unmuting user: {}", user_id.0);
