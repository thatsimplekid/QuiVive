 ...
 *
 *  permissions_helper.rs */

use crate::{ConnectionPool, PermissionType, QVError};
...
pub async fn check_administrator(ctx: &Context, msg: &Message, user_id: Option<UserId>) -> Result<bool, Box<dyn std::error::Error + Send + Sync>> {
    ...
    let permissions = match channel
        .permissions_for_user(ctx, user_id.unwrap_or(msg.author.id)).await {
    ...
    if permissions.administrator() {
        ...
        msg.channel_id.say(ctx, QVError::PermissionError(PermissionType::SelfPerm("administrator"))).await?;
        Ok(false)
    }
}
...
pub async fn check_moderator(ctx: &Context, msg: &Message, user_id: Option<UserId>) -> Result<bool, Box<dyn std::error::Error + Send + Sync>> {
    let channel = msg.channel_id.to_channel(ctx).await.unwrap().guild().unwrap();
    let user_permissions = match channel.permissions_for_user(ctx, user_id.unwrap_or(msg.author.id)).await {
        Ok(permissions) => permissions,
        Err(_) => return Ok(false)
    };
    ...
    if is_admin {
        ...
        if user_id.is_none() && !mod_result {
            msg.channel_id.say(ctx, QVError::PermissionError(PermissionType::SelfPerm("moderator"))).await?;
        }
        Ok(mod_result)
    }
}
...
async fn check_moderator_internal(ctx: &Context, msg: &Message, user: &User) -> Result<bool, Box<dyn std::error::Error + Send + Sync>> {
    ...
    let data = sqlx::query!(
        "SELECT mod_role_id FROM guild_info WHERE guild_id = $1", msg.guild_id.unwrap().0 as i64
    ...
    let role_id = RoleId::from(data.mod_role_id.unwrap() as u64);
    let has_role = user.has_role(ctx, msg.guild_id.unwrap(), role_id).await.unwrap_or(false);
    Ok(has_role)
}